...
N=100000
#f = TFile.Open(INPUT_DIR+args.input)
#t = f.Get('t')
#N = t.GetEntries()
datasets = {
        'TTbar': 100#,
...
if N/events_per_jobs>100:
    events_per_jobs=int(round(N/100))
events_per_jobs=N
timestamp = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
timestamp = '{}_{}'.format(timestamp, args.name)
print ('Size of tree : {}, number of events per job : {}, number of jobs : {}'.format(N,events_per_jobs,round(N/events_per_jobs)))
print (timestamp)

for dataset in order:
    nevents = datasets[dataset]
    executable_path = os.path.abspath('../build/TTbar_fully_leptonic')
    output = "output_{}".format(dataset)

    slurm_config = copy.deepcopy(config)
    slurm_working_dir = os.path.join('slurm', timestamp, '{}'.format(dataset))
